...
def test_upload_full_text_parses_and_stores_pdf(client):
    project_id = create_project(client)
    citation_id = create_citation(client, project_id)

    response = upload_full_text(client, project_id, citation_id, make_pdf("Hello world"))
    ...
    detail = client.get(f"/review-projects/{project_id}/citations/{citation_id}").json()
    assert detail["full_text"]["parse_status"] == "parsed"
    assert detail["full_text"]["original_filename"] == "paper.pdf"


def test_full_text_can_be_attached_regardless_of_screening_decision(client):
    project_id = create_project(client)
    citation_id = create_citation(client, project_id)
    client.post(
    ...
    response = upload_full_text(client, project_id, citation_id, make_pdf())

    assert response.status_code == 201


def test_pdf_with_no_text_layer_is_flagged_not_errored(client):
    project_id = create_project(client)
    citation_id = create_citation(client, project_id)

    response = upload_full_text(client, project_id, citation_id, make_pdf(text=None))
...
def test_uploading_again_replaces_the_previous_full_text(client):
    project_id = create_project(client)
    citation_id = create_citation(client, project_id)

    upload_full_text(client, project_id, citation_id, make_pdf("First version"), filename="v1.pdf")
    second = upload_full_text(
        client, project_id, citation_id, make_pdf("Second version"), filename="v2.pdf"
    )
...
def test_replacing_a_parsed_full_text_with_an_unparseable_one_flags_it(client):
    project_id = create_project(client)
    citation_id = create_citation(client, project_id)

    upload_full_text(client, project_id, citation_id, make_pdf("Readable text"))
    second = upload_full_text(client, project_id, citation_id, make_pdf(text=None))
...
def test_download_full_text_returns_the_uploaded_pdf(client):
    project_id = create_project(client)
    citation_id = create_citation(client, project_id)
    pdf_bytes = make_pdf("Downloadable content")
    upload_full_text(client, project_id, citation_id, pdf_bytes)
...
def test_download_full_text_returns_404_when_none_uploaded(client):
    project_id = create_project(client)
    citation_id = create_citation(client, project_id)
...
def test_citation_detail_has_no_full_text_before_upload(client):
    project_id = create_project(client)
    citation_id = create_citation(client, project_id)
...
def test_upload_full_text_rejects_non_pdf_file(client):
    project_id = create_project(client)
    citation_id = create_citation(client, project_id)
...
def test_upload_full_text_for_missing_citation_returns_404(client):
    project_id = create_project(client)
