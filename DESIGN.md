...
App shell: a 200px left project rail, then the main area. Every page of a Review Project sits in it, and each section of the project is its own route. The project name in the shell header is plain text, not a heading, so each page's own title is its only h1. Screening uses a folio strip ("412 of 1,860" with a thin progress line), a reading page with a 62ch column, and a 240px sticky margin for PICO criteria and the AI suggestion. At 70rem (1120px) and narrower the rail collapses to a header with a Menu button that reveals the same links, so navigation stays reachable, and below 56rem the margin drops below the page. The page frame is 80rem wide when the rail is present, so the reading column keeps its full 62ch. Spacing follows a 4px base with a large step between sections (40px and 64px) and a small step inside components (4px to 16px). Density is compact in queues and tables and generous on the reading page. The grid breaks on purpose in one place: the sealed co-reviewer tab, which sits in the margin until the reviewer commits.
...
| 2026-09-19 | The 200px project rail was not built in the app at first | The app had no separate routes for Criteria, Search terms, Conflicts and the rest, so a rail had nothing to link to. In the standalone preview the rail collapsed to a Menu button on narrow screens instead of hiding, so navigation stayed reachable. Built in #46, see the rows below. |
...
| 2026-09-19 | Each section of a Review Project is its own route under a shared layout with the rail (#46) | Criteria, Search terms, Citations, Duplicates, Conflicts, Extraction fields, Invitations and the PRISMA flow are separate pages, so the rail has real links and a section can be linked to. The project loads once in the layout and every section reads it, so moving between sections costs no extra requests. |
| 2026-09-19 | The AI suggestion never fills the screening decision or its reason, and it loads after the abstract (#49) | It used to arrive with the citation, pre-select its decision and copy its reasoning into the Reason field, so one Ctrl+Enter recorded the AI's call as the reviewer's own. Now the abstract and form show at once, the margin says "Preparing an AI suggestion…" until it is ready, and the reviewer's ink is only ever the reviewer's. The full-text decision and extraction values are still pre-filled from their suggestion, to be revisited separately. |
| 2026-09-19 | The rail folds into a Menu button at 70rem (1120px) and narrower, with one breakpoint | With the rail visible, the reading page (62ch column plus a 240px margin) needs more room than a tablet has. Folding the rail away first keeps the reading page's own 56rem stacking rule valid and adds one breakpoint to test. A landscape iPad (1024px) gets the Menu button. |
| 2026-09-19 | The page frame and top bar widen from 72rem to 80rem when the rail is present | At 72rem the rail and its gap left the reading column about 30px short of its 62ch cap at every viewport, because the frame caps the width. Measured in a real browser, the abstract column is a full 556px (62ch) at 1120, 1280 and 1440px. The top bar widens with the frame so the brand lines up with the rail. |
| 2026-09-19 | The project name in the shell header is a paragraph, not a heading | The shell header would put an h1 on every page next to each page's own title. Section pages show their panel title as an h2, so they currently have no h1. Promoting the panel titles is left as a follow-up. |
| 2026-09-19 | The Full-Text Decision has no default, and saving without a choice asks for one | Same stray-click risk as screening, and a Save with Maybe pre-selected records a real decision. Supersedes the last sentence of the row above ("The full-text decision form still defaults to Maybe..."). Built in #51. |
| 2026-09-19 | Extraction Values start empty, with the AI value beside each field and a Use action | Per-field Save must only record what the Reviewer typed or explicitly accepted. Use copies the AI value into the input; Save still records it. Built in #51. |
| 2026-09-19 | The Full-Text Suggestion loads after the page, on its own request | The page must not wait on a model call that reads the whole PDF, and a late arrival fills nothing. A suggestion asked for before a PDF was replaced is dropped. Built in #51. |
